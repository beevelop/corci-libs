...
Archiver.extract = function (zipfile, localdir) {
    zipfile = path.resolve(zipfile);
    localdir = path.resolve(localdir);

    P.all([
        fs.ensureDirAsync(localdir),
        fs.ensureFileAsync(zipfile)
    ]).then(function () {
        //@todo: progress??
        var stream = fs
    ...
    switch (_self.mode) {
        ...
        case 'unzip':
            exec('unzip -uo {0} -d {1} '.format(file, target), opts, function (err, stdout, stderr) {
                if (stdout) {
                    _self.log(build, Msg.debug, '{2}', stdout);
                }
                if (err || stderr) {
                    return _self.buildFailed(build, 'error executing unzip\n{2}\n{3}', err, stderr);
                }
                done();
            });
            break;
